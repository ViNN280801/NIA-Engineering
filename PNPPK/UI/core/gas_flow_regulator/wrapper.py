...
class GFR(IGFR):
    ...
    def __init__(self, port: str, baudrate: int, slave_id: int, timeout: int) -> None:
        ...
        """
        logger.debug(
            "Initializing GFR with port=%s, baudrate=%d, slave_id=%d, timeout=%d",
            port,
            baudrate,
            slave_id,
            timeout,
    ...
    def connect(self) -> bool:
        ...
        """
        logger.info(
            "Attempting to initialize GFR device with config: port=%s, baudrate=%d, slave_id=%d, timeout=%d",
        ...
        if not self._handle:
            logger.error(
                "GFR_Init did not create a valid handle. Last error: %s",
                self.get_last_error(),
            )
            return False
        result = gfr_lib.GFR_Init(
    ...
    def set_flow(self, setpoint: float) -> bool:
        ...
        if result != 0:
            logger.error(
                "Failed to set flow to %.3f SCCM. Error: %s",
                setpoint,
                self.get_last_error(),
            )
        else:
    ...
    def get_flow(self) -> float:
        ...
        if result != 0:
            logger.error(
                "Failed to retrieve flow (return code %d). Error: %s",
                result,
                self.get_last_error(),
            )
            return -1.0
    ...
    def set_gas(self, gas_id: int) -> bool:
        ...
        if result != 0:
            logger.error(
                "Failed to set gas to ID %d. Error: %s", gas_id, self.get_last_error()
            )
        else:
    ...
    def close(self) -> None:
        ...
        if self._handle and self._handle.modbus_ctx:
            ...
            logger.warning(
                "Attempted to close GFR device, but handle is invalid or already closed."
            )
